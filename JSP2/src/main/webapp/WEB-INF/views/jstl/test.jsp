...
<body>
    ...
    * 주의사항 : else 구문이 없다!
	...
	<c:set var="test3" value="양념 닭갈비"></c:set>
		...
		<li>
			<%--
				...
				${fn:length(컬렉션|배열|문자열)}
					->컬렉션|배열|문자열 의 길이를 반환
			 --%>
			nameList의 길이(저장된 데이터개수) : ${fn:length(nameList)}
		</li>
	...
	<ul>
		<c:forEach items="${nameList}" varStatus="vs">
		
			<c:if test="${vs.first}"> <%-- 반복문의 첫번째 순회일때 true --%>
				<li>------------시작------------</li>
			</c:if>
			...
			<c:if test="${vs.last}"> <%-- 반복문의 마지막 순회일때 true --%>
				<li>------------종료------------</li>
			</c:if>
